...
app.post("/attack", async (req, res) => {
    ...
    if (레어몬스터이름) {
        몬스터 = 레어악마불러오기(클라이언트층, 레어몬스터이름);
    } else {
        if (Math.random() < 1 / 18000) {
            랜덤등장몬스터이름 = "Ξ. 바론";
            몬스터 = 레어악마불러오기(클라이언트층, 랜덤등장몬스터이름);
        } else if (Math.random() < 1 / 9000) {
            랜덤등장몬스터이름 = "Ω. 사탄";
            몬스터 = 레어악마불러오기(클라이언트층, 랜덤등장몬스터이름);
        } else if (Math.random() < 1 / 3000) {
            랜덤등장몬스터이름 = "DCCLXXVII. 벨제부브";
            몬스터 = 레어악마불러오기(클라이언트층, 랜덤등장몬스터이름);
        } else if (Math.random() < 1 / 900) {
            랜덤등장몬스터이름 = "LXXXIX. 레비아탄";
            몬스터 = 레어악마불러오기(클라이언트층, 랜덤등장몬스터이름);
        } else if (Math.random() < 1 / 450) {
            랜덤등장몬스터이름 = "LXVII. 메타트론";
            몬스터 = 레어악마불러오기(클라이언트층, 랜덤등장몬스터이름);
        } else if ((유저.경험치 ?? 0) % 11 === 0 && 유저.경험치 !== 0) {
            랜덤등장몬스터이름 = "CXLIV. 황금고블린";
            몬스터 = 레어악마불러오기(클라이언트층, 랜덤등장몬스터이름);
        } else {
            몬스터 = 일반악마불러오기(클라이언트층 || 1);
        }
    }

    유저.조우기록 = 유저.조우기록 || { 일반: 0, 레어: 0, 신화: 0, 고대: 0, 태초: 0, 공허: 0, 황금: 0 };
    ...
    // ✅ 보상 계산도 클라이언트층 기준
    const 기본보상 = 보상계산(클라이언트층, 유저, 몬스터);
    기본보상.숙련도 += 인사이트추가숙련도(유저);
    const 인텔리배율 = 인텔리전스추가경험치배율(유저); // 💡 새 함수
    기본보상.경험치 += Math.floor(기본보상.경험치 * 인텔리배율);
    ...
    if (드랍장비) {
        ...
        if (기존) {
            console.log("기존 장비 중복 감지:", 키, "기존 횟수:", 새유저.합성기록[키]);
            새유저.최대체력 += 1;
            새유저.합성기록[키] = (새유저.합성기록[키] || 0) + 1;
            console.log("→ 증가 후 합성횟수:", 새유저.합성기록[키]);
        } else {
            // 첫 획득일 때만 공격력 증가
            새유저.장비목록.push(드랍장비);
            새유저.합성기록[키] = 0;
...
app.post("/upgrade-item", async (req, res) => {
    ...
    if (성공) {
        ...
        if ((대상.강화 || 0) > 0) {
            ...
            // 감소량 계산: 실패 시 한 단계 감소하므로 그 단계의 강화 증가량만큼 줄임
            const 감소량 = 강화증가량맵[등급] || 0;
            대상.공격력 -= 감소량;
            유저.공격력 -= 감소량;

            메시지 = `강화 실패.. 강화 수치가 1 감소하고 공격력 -${감소량}`;
        } else {
            메시지 = "강화 실패..";
        }
    }
...
app.post("/gamble", async (req, res) => {
    ...
    const 장비맵 = {
        "레어": { 이름: "디아블로의 심장", 공격력: 5 },
        "신화": { 이름: "레비아탄의 비늘", 공격력: 10 },
        "고대": { 이름: "벨제부브의 꼬리", 공격력: 15 },
        "태초": { 이름: "사탄의 날개", 공격력: 20 },
        "공허": { 이름: "바론의 촉수", 공격력: 25 }
    };
...
function 보상계산(층, 유저, 몬스터) {
    ...
    if (몬스터.타입 === "황금") {
        추가골드 = 골드 * 4; // 골드 5배 중 4배는 추가분
        골드 *= 5;
    }

    if (몬스터.타입 === "계시") {
        추가숙련도 = 숙련도 * 4; // 숙련도 5배 중 4배는 추가분
        숙련도 *= 5;
    }
...
function 레어악마불러오기(층, 이름) {
    const 이름맵 = {
        "Ξ. 바론": { 체력계수: 3.0, 방어계수: 2.0, 타입: "공허" },
        "Ω. 사탄": { 체력계수: 2.6, 방어계수: 1.8, 타입: "태초" },
        "DCCLXXVII. 벨제부브": { 체력계수: 2.2, 방어계수: 1.6, 타입: "고대" },
        "LXXXIX. 레비아탄": { 체력계수: 1.8, 방어계수: 1.4, 타입: "신화" },
        "LXVII. 메타트론": { 체력계수: 1.6, 방어계수: 1.3, 타입: "계시" },
        "IV. 디아블로": { 체력계수: 1.4, 방어계수: 1.2, 타입: "레어" },
        "CXLIV. 황금고블린": { 체력계수: 0.8, 방어계수: 0.8, 타입: "황금" }
    };

    const 설정 = 이름맵[이름];
    if (!설정) return null;

    const 기준체력 = 층 === 1 ? 20 : (층 - 1) * 500
    const 기준방어력 = 1;

    return {
        이름,
        체력: Math.floor(기준체력 * 설정.체력계수),
        방어력: Math.floor(기준방어력 * 설정.방어계수),
        타입: 설정.타입
    };
}
