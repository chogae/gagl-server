...
app.post("/register-user", async (req, res) => {
    ...
    const 삽입값 = {
        ...
        우편함: [
            ...
            { 이름: "사탄의 날개", 수량: 4, 날짜, 메모: "신규유저 이벤트보상. 무기화면에서 합성을 진행하세요" },
...
app.post("/receive-mail", async (req, res) => {
    ...
    const { error: updateErr } = await supabaseAdmin
        .from("users")
        .update({ 유물목록, 장비목록, 우편함, 장비공격력: 유저.장비공격력, 최종공격력: 유저.최종공격력, 햄버거현질: 유저.햄버거현질 })
        .eq("유저UID", 유저UID);
    ...
    return res.json({ 유물목록, 장비목록, 우편함, 장비공격력: 유저.장비공격력, 최종공격력: 유저.최종공격력, 햄버거현질: 유저.햄버거현질 });
});
...
function 마왕전전투시뮬레이션(도전자, 마왕) {
    ...
    const 도전자카드세우스 = 유저.악세사리목록?.find(a => a.이름 === "카드세우스" && a.장착 === 1);
    const 도전자카드세우스보정 = 도전자카드세우스 ? Math.max(0.7, 1 - 0.05 * 도전자카드세우스.등급) : 1;


    const 마왕카드세우스 = 유저.악세사리목록?.find(a => a.이름 === "카드세우스" && a.장착 === 1);
    const 마왕카드세우스보정 = 마왕카드세우스 ? Math.max(0.7, 1 - 0.05 * 마왕카드세우스.등급) : 1;

    마왕.최대체력 = Math.round(마왕.최대체력 * 1000 * 도전자카드세우스보정);
    도전자.최대체력 = Math.round(도전자.최대체력 * 1000 * 마왕카드세우스보정);
    ...
    while (도전자HP > 0 && 마왕HP > 0) {
        ...
        const 기본데미지 = Math.floor(도전자.최종공격력 * 랜덤보정 * 크리 * 버서커 * 버닝 * 마술);
...
function 전투시뮬레이션(유저, 몬스터, 전투로그, 시작턴, 보스전 = false) {
    ...
    몬스터.체력 = Math.floor(몬스터.체력 * 보정 * 브로큰하트 * 산산조각하트 * 카드세우스보정);
    let 몬스터HP = 몬스터.체력;
    let 현재턴 = 시작턴;
...
function 최대체력계산(유저) {
    ...
    const 음양감소배율 = 음양 ? Math.max(0.4, 1 - 0.1 * 음양.등급) : 1;


    const 스킬 = 유저.스킬 || {};
    const 홀리워터 = 스킬["홀리워터"] || 0;

    return Math.round((10 + 추가체력 + 홀리워터) * 하트플러스 * 하트비트 * 소망 * 물약 * 하트윙 * 테크놀로지아 * 음양감소배율);
}
