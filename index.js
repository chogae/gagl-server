...
app.post("/get-user", async (req, res) => {
    ...
    const 최종공격력 = ((data.레벨공격력 || 10) + (data.장비공격력 || 0)) * (전직카운트 || 1);
    // ⭐ 최종공격력
    if (data.최종공격력 !== 최종공격력) {
        업데이트필요 = true;
        업데이트값.최종공격력 = 최종공격력;
...
app.post("/attack-normal", async (req, res) => {
    ...
    let 레어몬스터이름 = null;
    if (Math.random() < 1 / 7000) 레어몬스터이름 = "루시퍼";
...
app.post("/attack-rare", async (req, res) => {
    ...
    if (드랍장비) {
        ...
        if (기존) {
            ...
            새유저.장비공격력 += 드랍장비.공격력;
            // 새유저.최종공격력 += 드랍장비.공격력;
            새유저.최종공격력 = ((새유저.레벨공격력 || 10) + (새유저.장비공격력 || 0)) * (새유저.전직공격력 || 1);
        }
    }
    ...
    // ✅ 레어몬스터 등장 확률 추가
    let 새로운레어몬스터이름 = null;
    if (Math.random() < 1 / 7000) 새로운레어몬스터이름 = "루시퍼";
...
app.post("/upgrade-item", async (req, res) => {
    ...
    유저.최종공격력 = ((유저.레벨공격력 || 10) + (유저.장비공격력 || 0)) * (유저.전직공격력 || 1);
...
app.post("/ranking", async (req, res) => {
    ...
    try {
        ...
        const { data: 유저들, error } = await supabaseAdmin
            .from("users")
            .select("유저아이디, 레벨, 공격력, 현재층, 장비목록, 합성기록, 전직정보, 최종공격력")
            .eq("버전업", 4)
            .not("최종공격력", "is", null)
...
app.post("/promote-job", async (req, res) => {
    ...
    const 최종공격력 = ((유저.레벨공격력 || 10) + (유저.장비공격력 || 0)) * ((완료전직갯수 + 1) || 1);
...
function 공격스킬적용(유저, 현재유저HP) {
    ...
    const 버서커계열 = [
        ...
        { 이름: "베타 버서커", 배율: 2.6, 소모: 2 },
        { 이름: "감마 버서커", 배율: 4.2, 소모: 3 },
        { 이름: "델타 버서커", 배율: 6.0, 소모: 4 },
        { 이름: "엡실론 버서커", 배율: 8.0, 소모: 5 },
        { 이름: "제타 버서커", 배율: 10.2, 소모: 6 },
        { 이름: "에타 버서커", 배율: 12.6, 소모: 7 },
        { 이름: "쎄타 버서커", 배율: 15.2, 소모: 8 },
        { 이름: "아이오타 버서커", 배율: 18.0, 소모: 9 },
        { 이름: "카파 버서커", 배율: 21.0, 소모: 10 },
        { 이름: "람다 버서커", 배율: 24.2, 소모: 11 },
        { 이름: "뮤 버서커", 배율: 27.6, 소모: 12 },
        { 이름: "뉴 버서커", 배율: 31.2, 소모: 13 },
        { 이름: "크사이 버서커", 배율: 35.0, 소모: 14 },
        { 이름: "오미크론 버서커", 배율: 39.0, 소모: 15 },
        { 이름: "파이 버서커", 배율: 43.2, 소모: 16 },
        { 이름: "로 버서커", 배율: 47.6, 소모: 17 },
        { 이름: "시그마 버서커", 배율: 52.2, 소모: 18 },
        { 이름: "타우 버서커", 배율: 57.0, 소모: 19 },
        { 이름: "윕실론 버서커", 배율: 62.0, 소모: 20 },
        { 이름: "카이 버서커", 배율: 67.2, 소모: 21 },
        { 이름: "프사이 버서커", 배율: 72.6, 소모: 22 },
        { 이름: "오메가 버서커", 배율: 78.2, 소모: 23 }
    ];
...
const 유물데이터 = {
    ...
    "모루": { 설명: "장비 강화확률을 증가시킵니다", 효과: "장비강화확률증가", 배율: 0.001 },
    "클로버": { 설명: "장비 드랍율을 증가시킵니다", 효과: "장비드랍율증가", 배율: 0.001 },
