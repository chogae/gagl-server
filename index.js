...
app.post("/attack-normal", async (req, res) => {
    ...
    if (추가회복 > 0) {
        ...
        전투로그.push({
            ...
            효과: `체력을 ${유저.남은체력 - 회복전체력} 회복합니다`,  // "체력 회복"만 표시
        });
    }
    ...
    let 레어몬스터이름 = null;
    if (Math.random() < 1 / 18000) 레어몬스터이름 = "Ξ. 바론";
    else if (Math.random() < 1 / 9000) 레어몬스터이름 = "Ω. 사탄";
    else if (Math.random() < 1 / 3000) 레어몬스터이름 = "DCCLXXVII. 벨제부브";
    else if (Math.random() < 1 / 900) 레어몬스터이름 = "LXXXIX. 레비아탄";
    else if (Math.random() < 1 / 450) 레어몬스터이름 = "LXVII. 메타트론";
    else if ((유저.경험치 ?? 0) % 11 === 0 && 유저.경험치 !== 0) 레어몬스터이름 = "CXLIV. 황금고블린";
