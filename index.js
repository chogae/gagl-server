...
app.post("/attack-boss", async (req, res) => {
    ...
    const 발동확률 = Math.min(0.99, 암포라 * 0.01);
...
app.post("/attack-normal", async (req, res) => {
    ...
    const 확률 = Math.min(0.099, 모래시계개수 * 0.001);
...
function 데미지계산(유저, 몬스터, 스킬결과) {
    ...
    const 보정 = Math.max(0.01, 1 - 0.01 * 쉴드밴개수);
    const 보정투 = Math.max(0.01, 1 - 0.01 * 쉴드배앤개수);
...
function 전투시뮬레이션(유저, 몬스터, 전투로그, 시작턴, 보스전 = false) {
    ...
    while (유저HP > 0 && (보스전 || 몬스터HP > 0)) {
        ...
        const 스탭개수 = 유저.유물목록?.["스탭"] || 0;
        const 무시확률 = 스탭개수 * 0.001;
...
function 레어몬스터등장판정(유저) {
    ...
    const 고스트보정 = Math.min(1.99, 1 + 0.01 * 고스트개수);

    const 고오스개수 = 유저.유물목록?.["고오스"] || 0;
    const 고오스보정 = Math.min(1.99, 1 + 0.01 * 고오스개수);

    const 고오스트개수 = 유저.유물목록?.["고오스트"] || 0;
    const 고오스트보정 = Math.min(1.99, 1 + 0.01 * 고오스트개수);
