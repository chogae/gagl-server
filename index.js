...
app.post("/upgrade-item", async (req, res) => {
    ...
    // // ✅ 유저 최종 공격력 재계산 (레벨기반 초기공격력 + 장비합산)
    // const 기본공격력 = 10 + ((유저.레벨 || 1) - 1); // 1레벨 = 10, 2레벨 = 11...
    // const 장비공격력합 = (장비목록 || []).reduce((합, 장비) => 합 + (장비.공격력 || 0), 0);
    // const 최종공격력 = 기본공격력 + 장비공격력합;
...
app.post("/update-skill", async (req, res) => {
    ...
    const 스킬정보 = {
        ...
        브로큰: { 단계: Array(2).fill(0) },
        즉사: { 단계: Array(2).fill(0) },
    };
...
function 공격스킬적용(유저, 현재유저HP) {
    ...
    const 유저조우카운트 = 유저.조우카운트 || 0;

    for (let i = 0; i < 즉사조건.length; i++) {
        if (즉사레벨 >= 즉사조건[i].레벨) {
            if ((유저조우카운트) % 즉사조건[i].주기 === 0) {
                const 발동확률 = 즉사레벨 * 0.1; // 예: 5레벨이면 50% 확률
                if (Math.random() < 발동확률) {
                    결과.즉사발동 = true;
                }
                break;
