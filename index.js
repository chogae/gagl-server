...
app.post("/promote-job", async (req, res) => {
    ...
    return res.json({ 성공: true, 전직정보: 전직정보, 경험치: 경험치 - 비용, 레벨: 새레벨, 최종공격력: 최종공격력, 레벨공격력: 새레벨공격력 });
});
...
app.post("/gamble-Equipment", async (req, res) => {
    ...
    try {
        ...
        for (let i = 0; i < 횟수; i++) {
            ...
            await 로그기록(유저.유저아이디, `${드랍장비.이름} 획득`);
