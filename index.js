...
// index.js (또는 서버 진입 파일)에 다음 코드를 추가하거나 기존 handler를 대체하세요.

// app.post("/synthesize-item", async (req, res) => {
//     const { 유저UID } = req.body;
//     if (!유저UID) {
//         // 클라이언트에서는 결과.오류를 체크하므로, 키를 "오류"로 맞춰줍니다.
//         return res.status(400).json({ 오류: "유저UID 누락" });
//     }

//     // 1) 유저 정보 조회
//     const { data: 유저, error: selectErr } = await supabaseAdmin
//         .from("users")
//         .select("*")
//         .eq("유저UID", 유저UID)
//         .single();
...
//     // 2) 현재 장비목록과 합성기록 복제
//     let 장비목록 = Array.isArray(유저.장비목록) ? [...유저.장비목록] : [];
//     let 합성기록 = { ...(유저.합성기록 || {}) };

//     // 3) 등급 순서 정의
//     const 등급순서 = ["일반", "레어", "신화", "고대", "태초", "타락"];
//     //  등급별 기본 이름·공격력 맵도 동일하게 정의
//     const 장비맵 = {
//         "일반": { 이름: "릴리트의 독니", 공격력: 30 },
//         "레어": { 이름: "디아블로의 뿔", 공격력: 63 },
//         "신화": { 이름: "레비아탄의 비늘", 공격력: 132 },
...
//     // 4) 합성할 하위 등급 찾기 (개수가 3개 이상인 가장 낮은 등급)
//     let 합성할하위등급 = null;
//     for (const grade of 등급순서) {
//         const count = 장비목록.filter(item => item.등급 === grade).length;
//         if (count >= 3) {
//             합성할하위등급 = grade;
//             break;
//         }
...
//     if (!합성할하위등급) {
//         return res.status(400).json({ 오류: "합성할 장비가 부족합니다." });
//     }

//     // 5) 하위 등급 장비 3개 제거
//     const 제거인덱스 = [];
//     let 제거카운트 = 0;
//     for (let i = 0; i < 장비목록.length; i++) {
//         if (장비목록[i].등급 === 합성할하위등급) {
//             제거인덱스.push(i);
//             제거카운트++;
//             if (제거카운트 === 3) break;
//         }
//     }
//     // 배열 인덱스를 내림차순으로 정렬한 뒤 splice
//     제거인덱스.sort((a, b) => b - a).forEach(idx => 장비목록.splice(idx, 1));

//     // 6) 다음 등급 계산
//     const lowerIndex = 등급순서.indexOf(합성할하위등급);
//     const nextIndex = lowerIndex + 1;
//     if (nextIndex >= 등급순서.length) {
//         // 이미 최상위 등급이라면 합성 불가
//         return res.status(400).json({ 오류: "최고 등급 장비는 더 이상 합성할 수 없습니다." });
//     }
//     const 합성후등급 = 등급순서[nextIndex];
//     const { 이름: 합성후이름, 공격력: 합성후기본공격력 } = 장비맵[합성후등급];

//     // 7) 합성 후 등급 장비가 이미 존재하는지 확인
//     const existIdx = 장비목록.findIndex(e => e.이름 === 합성후이름 && e.등급 === 합성후등급);
//     const key = `${합성후이름}|${합성후등급}`;

//     if (existIdx >= 0) {
//         // 이미 같은 이름·등급 장비가 있는 경우
//         합성기록[key] = (합성기록[key] || 0) + 1;
//         // 공격력 = (합성횟수 + 1) × 기본공격력
//         장비목록[existIdx].공격력 = (합성기록[key] + 1) * 합성후기본공격력;
//     } else {
//         // 신규 장비 추가
//         합성기록[key] = 0;
//         장비목록.push({
...
//     // 8) 장비공격력, 최종공격력 재계산
//     const 장비공격력 = 장비목록.reduce((sum, j) => sum + (j.공격력 || 0), 0);
//     유저.장비목록 = 장비목록;
//     유저.합성기록 = 합성기록;
//     유저.장비공격력 = 장비공격력;
//     // 최종공격력계산(user) 함수는 index.js 내에 이미 정의되어 있다고 가정
//     const 최종공격력 = 최종공격력계산(유저);
//     유저.최종공격력 = 최종공격력;
...
//     if (updateErr) {
//         console.error("장비 합성 후 DB 업데이트 실패:", updateErr);
//         return res.status(500).json({ 오류: "서버 오류가 발생했습니다. 잠시 후 다시 시도해주세요." });
//     }

//     // 10) 성공 응답: 클라이언트가 기대하는 JSON 구조
//     return res.json({
//         장비목록: 장비목록,
//         합성기록: 합성기록,
//         장비공격력: 장비공격력,
...
function 최대체력계산(유저) {
    ...
    // ✅ "태초" 또는 "타락" 등급만 필터링하여 합산
    const 총합성횟수 = Object.entries(합성기록)
        .filter(([키, _]) => 키.includes("|태초") || 키.includes("|타락"))
        .reduce((합, [_, 값]) => 합 + 값, 0);
