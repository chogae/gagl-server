...
app.post("/attack-normal", async (req, res) => {
    ...
    let 레어몬스터이름 = null;
    if (Math.random() < 1 / 1800) 레어몬스터이름 = "바론";
    else if (Math.random() < 1 / 900) 레어몬스터이름 = "사탄";
    else if (Math.random() < 1 / 300) 레어몬스터이름 = "벨제부브";
    else if (Math.random() < 1 / 90) 레어몬스터이름 = "레비아탄";
    else if (Math.random() < 1 / 45) 레어몬스터이름 = "숙고블린";
    else if ((유저.경험치 ?? 0) % 11 === 0 && 유저.경험치 !== 0) 레어몬스터이름 = "황금고블린";
...
app.post("/attack-rare", async (req, res) => {
    ...
    // ✅ 레어몬스터 등장 확률 추가
    let 새로운레어몬스터이름 = null;
    if (Math.random() < 1 / 1800) 새로운레어몬스터이름 = "바론";
    else if (Math.random() < 1 / 900) 새로운레어몬스터이름 = "사탄";
    else if (Math.random() < 1 / 300) 새로운레어몬스터이름 = "벨제부브";
    else if (Math.random() < 1 / 90) 새로운레어몬스터이름 = "레비아탄";
    else if (Math.random() < 1 / 45) 새로운레어몬스터이름 = "숙고블린";
    else if ((새유저.경험치 ?? 0) % 11 === 0 && 새유저.경험치 !== 0) 새로운레어몬스터이름 = "황금고블린";
...
app.post("/내전직정보조회", async (req, res) => {
    ...
    try {
        ...
        if (error || !data) {
            return res.status(404).json({ 메시지: "유저 정보를 찾을 수 없습니다." });
        }
        ...
        return res.status(500).json({ 메시지: "서버 오류 발생" });
    }
});

app.post("/전직요청", async (req, res) => {
    const { 유저UID, 직위 } = req.body;

    if (!유저UID || !직위) {
        return res.status(400).json({ 메시지: "유저UID와 직위가 필요합니다." });
    }
    ...
    try {
        ...
        // 이미 전직한 직위인지 확인
        if (전직정보[직위] === 1) {
            return res.status(400).json({ 메시지: "이미 전직한 직위입니다." });
        }

        // 경험치 부족 확인
        if (현재경험치 < 전직비용) {
            return res.status(400).json({ 메시지: "경험치가 부족합니다." });
        }

        // 전직정보 업데이트
        전직정보[직위] = 1;
        const 새경험치 = 현재경험치 - 전직비용;
        ...
        if (업데이트에러) {
            return res.status(500).json({ 메시지: "전직 업데이트 실패" });
        }
...
app.post("/gamble", async (req, res) => {
    ...
    const 장비맵 = {
        "태초": { 이름: "사탄의 날개", 공격력: 300 },
        "공허": { 이름: "바론의 촉수", 공격력: 400 }
    };
...
function 공격스킬적용(유저, 현재유저HP) {
    ...
    // 버서커
    const 버서커계열 = [
        { 이름: "앵거 버서커", 배율: 1.2, 소모: 1 },       // +0.0
        { 이름: "레이지 버서커", 배율: 2.6, 소모: 2 },     // +1.4
        { 이름: "프렌지 버서커", 배율: 4.2, 소모: 3 },     // +1.6
        { 이름: "매드니스 버서커", 배율: 6.0, 소모: 4 },   // +1.8
        { 이름: "하울 버서커", 배율: 8.0, 소모: 5 },       // +2.0
        { 이름: "새비지 버서커", 배율: 10.2, 소모: 6 },    // +2.2
        { 이름: "블러드 버서커", 배율: 12.6, 소모: 7 },    // +2.4
        { 이름: "카오스 버서커", 배율: 15.2, 소모: 8 },    // +2.6
        { 이름: "데스 버서커", 배율: 18.0, 소모: 9 },      // +2.8
        { 이름: "루나틱 버서커", 배율: 21.0, 소모: 10 }    // +3.0
    ];
    ...
    const 브로큰계열 = [
        { 이름: "쉴드 브로큰", 무시율: 0.1 },  // 10%
        { 이름: "배리어 브로큰", 무시율: 0.2 }, // 20%
        { 이름: "포트리스 브로큰", 무시율: 0.3 } // 30%
    ];
...
function 데미지계산(유저, 몬스터, 스킬결과) {
    ...
    // 🟡 브로큰 방어무시율 적용
    if (스킬결과.방어무시율 > 0) {
        방어력 = Math.floor(방어력 * (1 - 스킬결과.방어무시율));
    }
...
function 전투시뮬레이션(유저, 몬스터, 전투로그, 시작턴) {
    ...
    while (유저HP > 0 && 몬스터HP > 0) {
        ...
        const 브로큰적용방어력 = (스킬.방어무시율 > 0)
            ? Math.floor((몬스터.방어력 || 0) * (1 - 스킬.방어무시율))
            : (몬스터.방어력 || 0);
...
function 장비드랍판정(몬스터) {
    ...
    const 고정드랍 = {
        ...
        "신화": { 이름: "레비아탄의 비늘", 공격력: 100 },
        "고대": { 이름: "벨제부브의 꼬리", 공격력: 200 },
        "태초": { 이름: "사탄의 날개", 공격력: 300 },
        "공허": { 이름: "바론의 촉수", 공격력: 400 }
    };
...
function 보상계산(층, 유저, 몬스터) {
    ...
    if (몬스터.타입 === "황금") {
        추가골드 = 골드 * 99; // 골드 5배 중 4배는 추가분
        골드 *= 100;
    }

    if (몬스터.타입 === "계시") {
        추가숙련도 = 숙련도 * 99; // 숙련도 5배 중 4배는 추가분
        숙련도 *= 100;
    }
...
function 레어악마불러오기(층, 이름) {
    const 이름맵 = {
        "바론": { 체력계수: 3.0, 방어계수: 2.0, 타입: "공허" },
        "사탄": { 체력계수: 2.6, 방어계수: 1.8, 타입: "태초" },
        "벨제부브": { 체력계수: 2.2, 방어계수: 1.6, 타입: "고대" },
        "레비아탄": { 체력계수: 1.8, 방어계수: 1.4, 타입: "신화" },
        "숙고블린": { 체력계수: 1.6, 방어계수: 1.3, 타입: "계시" },
        "디아블로": { 체력계수: 1.4, 방어계수: 1.2, 타입: "레어" },
...
const 유물데이터 = {
    "소드": { 설명: "공격력이 증가합니다", 효과: "공격력증가", 배율: 0.01 },
    "하트플러스": { 설명: "체력이 증가합니다", 효과: "체력증가", 배율: 0.01 },
    "고스트": { 설명: "히든몬스터 등장률이 증가합니다", 효과: "등장률증가", 배율: 0.01 },
    "쉴드밴": { 설명: "악마들의 방어력을 감소시킵니다", 효과: "방어력감소", 배율: 0.01 },
    "하트마이너스": { 설명: "악마들의 체력을 감소시킵니다", 효과: "체력감소", 배율: 0.01 },
    "엑스": { 설명: "크리티컬 피해량을 증가시킵니다", 효과: "크리배율증가", 배율: 0.01 },
    "암포라": { 설명: "미정", 효과: "미정", 배율: 0.01 },
    "모루": { 설명: "장비 강화확률을 증가시킵니다", 효과: "장비강화확률증가", 배율: 0.01 },
    "클로버": { 설명: "장비 드랍율을 증가시킵니다", 효과: "장비드랍율증가", 배율: 0.01 },
    "플라워": { 설명: "미정", 효과: "미정", 배율: 0.01 },
    "모래시계": { 설명: "미정", 효과: "미정", 배율: 0.01 },
    "퍼즐": { 설명: "미정", 효과: "미정", 배율: 0.01 },
    "로켓": { 설명: "미정", 효과: "미정", 배율: 0.01 },
    "표창": { 설명: "미정", 효과: "미정", 배율: 0.01 },
    "봄": { 설명: "미정", 효과: "미정", 배율: 0.01 },
    "여름": { 설명: "미정", 효과: "미정", 배율: 0.01 },
    "가을": { 설명: "미정", 효과: "미정", 배율: 0.01 },
    "겨울": { 설명: "미정", 효과: "미정", 배율: 0.01 },
