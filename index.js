...
app.post("/upgrade-void-item", async (req, res) => {
    const { 유저UID, 이름, 등급 } = req.body;
    if (!유저UID || !이름 || !등급) {
        return res.status(400).json({ 오류: "필수 값 누락" });
    }
    ...
    if (등급 !== "공허") {
        return res.status(400).json({ 오류: "공허 장비만 강화 가능합니다" });
    }
    if ((대상.수량 || 0) < 1) {
        return res.status(400).json({ 오류: "수량이 부족합니다" });
    }
...
//패치중
//매직서바이벌
app.post("/magic-survival", async (req, res) => {
    try {
        const { 유저UID } = req.body;
        ...
        const 매직서바이벌몬스터 = [
            ...
            "카이미", "알로세스", "샤르나크", "샤크", "사브낙", "라하브", "말포르스", "하우레스", "피닉스", "바알",
        ...
        const 몬스터이름 = 매직서바이벌몬스터[Math.floor(Math.random() * 매직서바이벌몬스터.length)];
        ...
        if (!유저.매직서바이벌) {
            ...
            if (업데이트에러) {
                return res.status(500).json({ 오류: "매직서바이벌 초기화 실패" });
            }

            return res.json({ 성공: true, 매직서바이벌: 기본셋팅 });
        }

        return res.json({ 성공: true, 매직서바이벌: 유저.매직서바이벌 });

    } catch (e) {
        console.error("매직서바이벌 오류:", e);
        return res.status(500).json({ 오류: "서버 내부 오류" });
    }
});
...
function 마왕전전투시뮬레이션(도전자, 마왕) {
    ...
    const 궁술무시확률 = [0.05, 0.10, 0.15, 0.20, 0.25, 0.40];
    const 마술배율 = [1.2, 1.4, 1.6, 1.8, 2.0, 2.8];
...
function 전투시뮬레이션(유저, 몬스터, 전투로그, 시작턴, 보스전 = false) {
    ...
    while (유저HP > 0 && (보스전 || 몬스터HP > 0)) {
        ...
        const 정조준레벨 = 스킬["정조준"] || 0;
        if (정조준레벨 >= 1) {
            발동아이콘.push("정조준아이콘");
            데미지 = Math.floor(데미지 * (1 + 0.0014 * 정조준레벨 * 현재턴));
        }
...
function 유물드랍판정(몬스터, 유저) {
    ...
    const 보스의알보유량 = 유저.유물목록?.["보스의알"] || 0;
    const 고대의유물보유량 = Math.min(유저.유물목록?.["고대의유물"] || 0, 99);

    const 기본확률 = 0.003;
    const 보정계수 = (1 + 0.001 * 보스의알보유량) * (1 + 0.01 * 고대의유물보유량);
    const 드랍확률 = 기본확률 * 보정계수;
