...
app.post("/update-skill", async (req, res) => {
    ...
    const 총투자 = Object.values(스킬).reduce((a, b) => a + b, 0);
    res.status(400).json({ 오류: 단계수 });

    if (행동 === "투자") {
        console.log("====== 투자 시도 ======");
        console.log(`스킬 이름: ${스킬이름}`);
        console.log(`현재레벨: ${현재레벨}`);
        console.log(`최대레벨: ${최대레벨}`);
        console.log(`숙련도: ${숙련도}`);
        console.log(`총투자: ${총투자}`);
...
function 공격스킬적용(유저, 현재유저HP) {
    ...
    const 버서커계열 = [
        ...
        { 이름: "람다 버서커", 배율: 24.0, 소모: 11 },
        { 이름: "뮤 버서커", 배율: 27.2, 소모: 12 },
        { 이름: "뉴 버서커", 배율: 30.6, 소모: 13 },
        { 이름: "크사이 버서커", 배율: 34.2, 소모: 14 },
        { 이름: "오미크론 버서커", 배율: 38.0, 소모: 15 },
        { 이름: "파이 버서커", 배율: 42.0, 소모: 16 },
        { 이름: "로 버서커", 배율: 46.2, 소모: 17 },
        { 이름: "시그마 버서커", 배율: 50.6, 소모: 18 },
        { 이름: "타우 버서커", 배율: 55.2, 소모: 19 },
        { 이름: "윕실론 버서커", 배율: 60.0, 소모: 20 },
        { 이름: "카이 버서커", 배율: 65.0, 소모: 21 },
        { 이름: "프사이 버서커", 배율: 70.2, 소모: 22 },
        { 이름: "오메가 버서커", 배율: 75.6, 소모: 23 }
    ];
